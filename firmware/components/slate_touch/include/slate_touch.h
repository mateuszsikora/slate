 ...
 * the reset line on the CH422G. The I²C bus, the expander and the lock that
 * serialises the expander all belong to slate_display, which brought them up
 * for the panel and lends them here rather than have two owners for one bus.
 *
 ...
 * here. `i2c_port` is an already-installed legacy `driver/i2c.h` master port,
 * `expander` a live CH422G handle, and `expander_lock` the mutex that serialises
 * its read-modify-written output register. All three are borrowed; none is
