...
Volume is sent as the percentage itself, one for one. A TX-8270 answers `MVL56` — 86 — while its own `NRI` reports `volmax="82"`, so the two are not the same number and neither is reliably the scale; a receiver clamps what it cannot do, and a predictable mapping is worth more than a guessed one.
