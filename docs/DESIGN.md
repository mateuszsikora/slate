...
writes flash. When persistence fails after activation, the submitted document
remains the active in-memory configuration and the previous persisted document
is preserved for the next boot. Successful replacements publish a `reloaded`
WebSocket event. Transient requests outside edit mode return
`409 edit_mode_required`; activation and persistence failures return
`500 apply_failed` and `500 store_failed`, respectively.
